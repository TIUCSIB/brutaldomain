...
const hardButton =
  "rounded-none border-2 border-slate-950 shadow-[3px_3px_0_0_#0f172a] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none";
  ...
  return (
    <div className="relative min-h-svh overflow-hidden bg-[#edf4ff] text-slate-950">
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(#c9dbff 2px, transparent 2px), linear-gradient(90deg, #c9dbff 2px, transparent 2px)",
      ...
      <div className="relative mx-auto flex min-h-svh max-w-6xl flex-col justify-center gap-10 px-4 py-10 lg:flex-row lg:items-center lg:gap-16 lg:px-8">
        <section className="max-w-xl space-y-6">
          ...
          <div>
            ...
            <p className="mt-4 max-w-lg text-base font-bold leading-7 text-slate-700">
              统一管理域名资产、DNS 解析记录、到期状态与账户配额。
              登录后进入控制台。
            </p>
          </div>
          ...
          <ul className="space-y-3">
            {[
              ...
              return (
                <li
                  key={item.text}
                  className="flex items-center gap-3 border-2 border-slate-950 bg-white px-4 py-3 text-sm font-black shadow-[3px_3px_0_0_#0f172a]"
                >
        ...
        <section className="w-full max-w-md border-4 border-slate-950 bg-white shadow-[8px_8px_0_0_#0f172a]">
          ...
          <div className="space-y-6 p-6 sm:p-8">
            ...
            {errorMessage ? (
              <div
                role="alert"
                className="border-2 border-slate-950 bg-[#fff0f3] px-4 py-3 text-sm font-bold text-red-700 shadow-[3px_3px_0_0_#ff5c7a]"
              >
                {errorMessage}
              </div>
            ) : null}
